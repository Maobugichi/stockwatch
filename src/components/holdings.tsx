...
interface HoldingsProp {
    ...
    submitDetails:(e:React.FormEvent<HTMLFormElement>,userChoice:any,userId:number,setUserChoice:any,setOpen:any,setQuery:any,setNotification:any) => {}
    type:string
    header:any
}
    ...
    return(
        <Dialog open={open} onOpenChange={setOpen} >
            <DialogTrigger asChild>
                <Button clicked={() => setOpen(prev => !prev)} className="">{header}</Button>
            </DialogTrigger>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>{type}</DialogTitle>
                </DialogHeader>
                <Form submitForm={(e:React.FormEvent<HTMLFormElement>) => submitDetails(e,userChoice,parseInt(userId.current),setUserChoice,setOpen,setQuery,setNotification)} className="grid gap-4">
                    <div>
                    ...
                    {type.includes('portfolio') && <div>
                        ...
                        <Input
                         ...
                         placeholder="e.g 10"
                         checkInput={(e:React.ChangeEvent<HTMLInputElement>) => handleUserChoice(e,setUserChoice)}
                        />
                    </div>}
                    {type.includes('portfolio') && <div>
